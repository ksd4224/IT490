...
$queueName = 'front-pass-response'; // Replace with the actual queue name
    ...
    $callback = function ($message) {
        ...
        // Check if the 'status' field is 'success'
                if (isset($decodedMessage['status']) && $decodedMessage['status'] === 'success') {
                        ...
                        header("Location: login2.php?pass=success");
                        exit(); // Ensure script termination after the header redirection
                } else {
                        header("Location: login2.php?pass_error=pass_no_success");
                        exit();
                }
        } else {
