...
def callback_and_insert(ch, method, properties, body):
    ...
    try:
        ...
        # Construct a success response message without updating the database
        success_message = {
            'status': 'success',
            'message': f'Successfully received weight update for {email} to {weight}',
            'user_data': {
                'email': email,
                'weight': weight,
        ...
        # Send the success message back to the 'weight.data' queue
        ch.basic_publish(
