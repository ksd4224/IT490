...
def get_user_nutrition_data(user_id, cursor):
    nutrition_sql = """
        SELECT nd.*
...
def get_all_users_posts(cursor):
    all_posts_sql = "SELECT post_id, user_id, post_content, created_at FROM posts"
...
def handle_workout(ch, method, properties, body, cursor, db_connection, channel):
    ...
    try:
        ...
        if user:
            ...
            # Fetch user meals
            meals_data = get_user_meals(user['user_id'], cursor)

            # Fetch user nutrition data
            nutrition_data = get_user_nutrition_data(user['user_id'], cursor)

            # Fetch all users' posts
            all_users_posts_data = get_all_users_posts(cursor)

            # Send success message back to the queue with user information, workouts, meals, nutrition data, posts, all users' posts, and user total
            success_message = {
                'status': 'success',
                ...
                'all_users_posts_data': all_users_posts_data
            }
            ch.basic_publish(
