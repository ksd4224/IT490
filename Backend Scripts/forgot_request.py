...
consume1_channel.queue_declare(queue='front-pass-request', durable=True)
consume1_channel.queue_bind(exchange='frontend-backend', queue='front-pass-request', routing_key='pass.request')

forward1_connection = pika.BlockingConnection(parameters)
forward1_channel = forward1_connection.channel()
forward1_channel.queue_declare(queue='back-pass-request', durable=True)
forward1_channel.queue_bind(exchange='backend-database', queue='back-pass-request', routing_key='pass.back')
...
consume1_channel.basic_consume(queue='front-pass-request', on_message_callback=callback1, auto_ack=True)

try:
    print('Forogt is [*] Waiting for messages. To exit press CTRL+C')
