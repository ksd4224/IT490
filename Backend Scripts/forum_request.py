...
consume1_channel.queue_declare(queue='front-forum-request', durable=True)
consume1_channel.queue_bind(exchange='frontend-backend', queue='front-forum-request', routing_key='forum.request')

forward1_connection = pika.BlockingConnection(parameters)
forward1_channel = forward1_connection.channel()
forward1_channel.queue_declare(queue='back-forum-request', durable=True)
forward1_channel.queue_bind(exchange='backend-database', queue='back-forum-request', routing_key='forum.back')
...
consume1_channel.basic_consume(queue='front-forum-request', on_message_callback=callback1, auto_ack=True)
