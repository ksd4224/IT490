...
consume1_channel.queue_declare(queue='front-workout-request', durable=True)
consume1_channel.queue_bind(exchange='frontend-backend', queue='front-workout-request', routing_key='workout.request')

forward1_connection = pika.BlockingConnection(parameters)
forward1_channel = forward1_connection.channel()
forward1_channel.queue_declare(queue='back-workout-request', durable=True)
forward1_channel.queue_bind(exchange='backend-database', queue='back-workout-request', routing_key='workout.back')
...
consume1_channel.basic_consume(queue='front-workout-request', on_message_callback=callback1, auto_ack=True)
