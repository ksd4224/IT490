...
channel.queue_declare(queue='data-goals-response', durable=True)
channel.queue_bind(exchange='backend-database', queue='data-goals-response', routing_key='goals.data')


#publishing rabbitmq data to frontend
bd_rbqm = pika.BlockingConnection(parameters)
bd_channel = bd_rbqm.channel()
bd_channel.queue_declare(queue='front-goals-response', durable=True)
bd_channel.queue_bind(exchange='frontend-backend', queue='front-goals-response', routing_key='goals.response')
...
channel.basic_consume(
        queue='data-goals-response', on_message_callback=callback, auto_ack=True)

try:
    print('Consuming Goals [*] Waiting for messages. To exit press CTRL+C')
