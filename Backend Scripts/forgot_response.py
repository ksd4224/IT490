...
channel.queue_declare(queue='data-pass-response', durable=True)
channel.queue_bind(exchange='backend-database', queue='data-pass-response', routing_key='pass.data')


#publishing rabbitmq data to frontend
bd_rbqm = pika.BlockingConnection(parameters)
bd_channel = bd_rbqm.channel()
bd_channel.queue_declare(queue='front-pass-response', durable=True)
bd_channel.queue_bind(exchange='frontend-backend', queue='front-pass-response', routing_key='pass.response')
...
channel.basic_consume(
        queue='data-pass-response', on_message_callback=callback, auto_ack=True)

try:
    print('Consuming Forgot [*] Waiting for messages. To exit press CTRL+C')
