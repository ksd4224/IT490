...
channel.queue_declare(queue='data-reg-response', durable=True)
channel.queue_bind(exchange='backend-database', queue='data-reg-response', routing_key='reg.data')


#publishing rabbitmq data to frontend
bd_rbqm = pika.BlockingConnection(parameters)
bd_channel = bd_rbqm.channel()
bd_channel.queue_declare(queue='front-reg-response', durable=True)
bd_channel.queue_bind(exchange='frontend-backend', queue='front-reg-response', routing_key='reg.response')
...
channel.basic_consume(
        queue='data-reg-response', on_message_callback=callback, auto_ack=True)

try:
    print('Consuming Registration [*] Waiting for messages. To exit press CTRL+C')
