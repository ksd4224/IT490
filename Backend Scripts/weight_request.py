...
consume1_channel.queue_declare(queue='front-weight-request', durable=True)
consume1_channel.queue_bind(exchange='frontend-backend', queue='front-weight-request', routing_key='weight.request')

forward1_connection = pika.BlockingConnection(parameters)
forward1_channel = forward1_connection.channel()
forward1_channel.queue_declare(queue='back-weight-request', durable=True)
forward1_channel.queue_bind(exchange='backend-database', queue='back-weight-request', routing_key='weight.back')
...
consume1_channel.basic_consume(queue='front-weight-request', on_message_callback=callback1, auto_ack=True)
