...
consume1_channel.queue_declare(queue='front-login-request', durable=True)
consume1_channel.queue_bind(exchange='frontend-backend', queue='front-login-request', routing_key='log.request')

forward1_connection = pika.BlockingConnection(parameters)
forward1_channel = forward1_connection.channel()
forward1_channel.queue_declare(queue='back-login-request', durable=True)
forward1_channel.queue_bind(exchange='backend-database', queue='back-login-request', routing_key='log.back')
...
consume1_channel.basic_consume(queue='front-login-request', on_message_callback=callback1, auto_ack=True)
