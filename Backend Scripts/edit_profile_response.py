...
channel.queue_declare(queue='data-profile-response', durable=True)
channel.queue_bind(exchange='backend-database', queue='data-profile-response', routing_key='profile.data')


#publishing rabbitmq data to frontend
bd_rbqm = pika.BlockingConnection(parameters)
bd_channel = bd_rbqm.channel()
bd_channel.queue_declare(queue='front-profile-response', durable=True)
bd_channel.queue_bind(exchange='frontend-backend', queue='front-profile-response', routing_key='profile.response')
...
channel.basic_consume(
        queue='data-profile-response', on_message_callback=callback, auto_ack=True)

try:
    print('Consuming Edit Profile [*] Waiting for messages. To exit press CTRL+C')
