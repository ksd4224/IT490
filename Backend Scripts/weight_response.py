...
channel.queue_declare(queue='data-weight-response', durable=True)
channel.queue_bind(exchange='backend-database', queue='data-weight-response', routing_key='weight.data')


#publishing rabbitmq data to frontend
bd_rbqm = pika.BlockingConnection(parameters)
bd_channel = bd_rbqm.channel()
bd_channel.queue_declare(queue='front-weight-response', durable=True)
bd_channel.queue_bind(exchange='frontend-backend', queue='front-weight-response', routing_key='weight.response')
...
channel.basic_consume(
        queue='data-weight-response', on_message_callback=callback, auto_ack=True)

try:
    print('Consuming Weight [*] Waiting for messages. To exit press CTRL+C')
