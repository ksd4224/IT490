...
channel.queue_declare(queue='data-forum-response', durable=True)
channel.queue_bind(exchange='backend-database', queue='data-forum-response', routing_key='forum.data')


#publishing rabbitmq data to frontend
bd_rbqm = pika.BlockingConnection(parameters)
bd_channel = bd_rbqm.channel()
bd_channel.queue_declare(queue='front-forum-response', durable=True)
bd_channel.queue_bind(exchange='frontend-backend', queue='front-forum-response', routing_key='forum.response')
...
channel.basic_consume(
        queue='data-forum-response', on_message_callback=callback, auto_ack=True)

try:
    print('Consuming Forum [*] Waiting for messages. To exit press CTRL+C')
