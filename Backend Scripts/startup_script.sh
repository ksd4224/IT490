...
secondary_dbs_IP=10.248.179.18
...
echo "Falling back on secondary database"
...
if [[ $primary_Database == *"64 bytes from $primary_dbs_IP:"* ]]; then
 ...
 elif [[ $bck_IP == $bck_IP ]]; then
     echo "Checking the status of the secondary database"
     check=$(systemctl status mysql.service)
     if [[ $check == *"active (running)"* ]]; then
...
if [[ $primary_rabbitmq == *"64 bytes from $primary_rbmq_IP:"* ]]; then
    echo "checking status of primary rabbitmq"
    ...
    if [[ check_one == *"active (running)"* ]]; then
        ...
        echo "rabbitmq host has been updated to $primary_rbmq_IP"
        cd /home/adam/Projects/Project490
        echo "launching listener"
        ./receiver.py
    else:
        echo "Failure in inputting primary rabbitmq ip address"
    fi
    ...
    if [[ check_one == *"active (running)"* ]]; then
        ...
        sed -i "s/\(parameters = pika.ConnectionParameters(host='\)[^']*\(.*\)/\1$new_host\2/" $file_reg
        echo "rabbitmq host has been updated to $secondary_rbmq_IP"
        cd /home/adam/Projects/Project490
        echo "launching listener"
        ./receiver.py
    else:
        echo "Failure in inputting secondary rabbitmq ip address"
    fi

elif [[ $tertiary_rabbitmq == *"64 bytes from $tertiary_rbmq_IP:"* ]]; then
    echo "Checking status of tertiary rabbitmq"
    check_three=$(systemctl --host $tertiary_rbmq status rabbitmq-server.service)
    if [[ check_three == *"active (running)"* ]]; then
        new_host=$tertiary_rbmq_IP
        sed -i "s/\(parameters = pika.ConnectionParameters(host='\)[^']*\(.*\)/\1$new_host\2/" $file_reg
        echo "rabbitmq host has been updated to $tertiary_rbmq_IP"
        cd /home/adam/Projects/Project490
        echo "launching listener"
        ./receiver.py
    else:
        echo "Failue in inputting tertiary rabbitmq ip address" 
    fi
else
