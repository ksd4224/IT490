...
consume1_channel.queue_declare(queue='front-goals-request', durable=True)
consume1_channel.queue_bind(exchange='frontend-backend', queue='front-goals-request', routing_key='goals.request')

forward1_connection = pika.BlockingConnection(parameters)
forward1_channel = forward1_connection.channel()
forward1_channel.queue_declare(queue='back-goals-request', durable=True)
forward1_channel.queue_bind(exchange='backend-database', queue='back-goals-request', routing_key='goals.back')
...
consume1_channel.basic_consume(queue='front-goals-request', on_message_callback=callback1, auto_ack=True)
