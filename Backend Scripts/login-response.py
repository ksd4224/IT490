...
consume2_channel.queue_declare(queue='data-login-response', durable=True)
consume2_channel.queue_bind(exchange='backend-database', queue='data-login-response', routing_key='log.data')

forward2_connection = pika.BlockingConnection(parameters)
forward2_channel = forward2_connection.channel()
forward2_channel.queue_declare(queue='front-login-response', durable=True)
forward2_channel.queue_bind(exchange='frontend-backend', queue='front-login-response', routing_key='log.response')
...
def callback2(ch, method, properties, body):
    ...
    forward2_channel.basic_publish(
        ...
        routing_key='log.response',
        body=json.dumps(data),
        properties=pika.BasicProperties(delivery_mode=2)
    )

consume2_channel.basic_consume(queue='data-login-response', on_message_callback=callback2, auto_ack=True)
