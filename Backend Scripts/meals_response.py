...
channel.queue_declare(queue='data-meals-response', durable=True)
channel.queue_bind(exchange='backend-database', queue='data-meals-response', routing_key='meals.data')


#publishing rabbitmq data to frontend
bd_rbqm = pika.BlockingConnection(parameters)
bd_channel = bd_rbqm.channel()
bd_channel.queue_declare(queue='front-meals-response', durable=True)
bd_channel.queue_bind(exchange='frontend-backend', queue='front-meals-response', routing_key='meals.response')
...
channel.basic_consume(
        queue='data-meals-response', on_message_callback=callback, auto_ack=True)

try:
    print('Consuming Meals [*] Waiting for messages. To exit press CTRL+C')
